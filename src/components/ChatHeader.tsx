...
import { getIdentityLabel } from '../utils/auth';
...
export function ChatHeader({ isConnected }: ChatHeaderProps) {
  const [identityLabel, setIdentityLabel] = useState(getIdentityLabel());
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setIdentityLabel(getIdentityLabel());
    }, 1000);
      ...
      <div className="max-w-4xl mx-auto flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="text-sm text-gray-600 dark:text-gray-400">
            You are: <span className="font-medium">{identityLabel}</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
